...
(async () => {
  ...
  if (window.location.protocol !== 'moz-extension:' || window.top !== window.self) {
  ...
  if (britiveBanner && britiveBanner.message) {
    ...
    const safeType = allowedTypes.includes(britiveBanner.messageType) ? britiveBanner.messageType : 'INFO';
    bannerEl.className = 'banner ' + safeType;
  }

  const requestId = new URLSearchParams(window.location.search).get('requestId');
  const storage = await browser.storage.local.get('pendingContainerRequests');
  ...
  try {
    ...
    document.getElementById('destination').textContent = dest.hostname + dest.pathname;
  } catch (e) {
  ...
  if (typeof browser.contextualIdentities === 'undefined') {
    ...
    listDiv.appendChild(buildMissingExtensionNotice('Container support requires the contextualIdentities API. Install Firefox Multi-Account Containers to enable it.'));
    return;
  }
  ...
  try {
    ...
    clearElement(listDiv);
    listDiv.appendChild(buildMissingExtensionNotice('Container tabs are not enabled. Install Firefox Multi-Account Containers or enable container tabs in Firefox settings.'));
    return;
  }
  ...
  document.getElementById('cancel').addEventListener('click', async () => {
    ...
    await browser.storage.local.set({ pendingContainerRequests: pendingRequests });
...
async function renderContainers(url, listDiv) {
  ...
  CONTAINER_COLORS.forEach((c, i) => {
    ...
    swatch.addEventListener('click', () => {
      colorRow.querySelectorAll('.color-option').forEach(s => s.classList.remove('selected'));
